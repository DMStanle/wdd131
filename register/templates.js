...
export function successTemplate({ name, count, total }) {
  return `
    ...
    <p>Thank you <strong>${name || 'Friend'}</strong> for registering.</p>
    <p>You have registered <strong>${count}</strong> participant${count === 1 ? '' : 's'} and owe <strong>$${total.toLocaleString()}</strong> in fees.</p>
  `;
}
